...
**avg_net_rank** | **int** | 3-season average NET ranking (basketball&#39;s ranking window) | [optional] 
**avg_rpi** | **int** | 5-season average RPI ranking — the value non-basketball feed cards display and the ranking filter compares against, so the card matches the filter that surfaced it. &#x60;last_rpi&#x60; remains for surfaces that show the latest value. | [optional] 
