...
Upload a download (PDF / XLSX / PPTX). One row per kind — uploading an existing kind
replaces its file (06.5 Replace).
...
> DeskAdminReportsResponse get_admin_desk_reports(status=status)
...
(draft/building/live/hidden) or the queue's (draft/published/hidden).
...
with winthrop_client_python.ApiClient(configuration) as api_client:
    ...
    try:
        api_response = api_instance.get_admin_desk_reports(status=status)
...
# **get_desk_reports**
> DeskReportsResponse get_desk_reports()
...
Drafts, building and hidden reports are never listed.
...
with winthrop_client_python.ApiClient(configuration) as api_client:
    ...
    try:
        api_response = api_instance.get_desk_reports()
...
becomes the note on the customer's pending card unless an explicit admin_note is given.
Only an ask that is `new` or `building` can be sent back.
