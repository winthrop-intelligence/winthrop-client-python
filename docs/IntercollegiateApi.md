...
with winthrop_client_python.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = winthrop_client_python.IntercollegiateApi(api_client)
    job_post_id = 56 # int | Central Jobs remote ID of job post to sync
    remote_job_post = winthrop_client_python.RemoteJobPost() # RemoteJobPost | 

    try:
        api_response = api_instance.sync_job_post(job_post_id, remote_job_post)
        print("The response of IntercollegiateApi->sync_job_post:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling IntercollegiateApi->sync_job_post: %s\n" % e)
```
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **job_post_id** | **int**| Central Jobs remote ID of job post to sync | 
