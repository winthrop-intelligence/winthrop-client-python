...
**client** | **str** | The account the report went live on; null marks a legacy admin-only report. | 
**audience_user_count** | **int** | \&quot;Visible to N people\&quot; — the active users the report reaches (Desk::Audience): the selected account&#39;s, narrowed to selected recipients when restricted. Recorded on the published version independently of whether notification email is enabled.  | 
**turnaround_label** | **str** |  | 
**requester_name** | **str** |  | 
**version** | [**DeskAdminVersion**](DeskAdminVersion.md) |  | 
**notified** | **bool** | Whether this publish queued the delivery email (async; not a delivery receipt). False, with notified_count 0 and no names, when the runtime notification switch is off, re-notify was not requested for an update, or no email could be queued.  | 
**notified_count** | **int** | Number of recipients in the queued notification email | 
