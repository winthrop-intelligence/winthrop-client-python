...
class GetFavorites200ResponseInner(BaseModel):
    ...
    favorites_category_id: Optional[StrictInt] = Field(default=None, description="Category ID (only when detailed=1)")
    category_name: Optional[StrictStr] = Field(default=None, description="Category name (only when detailed=1)")
    name: Optional[StrictStr] = Field(default=None, description="Favoritable record name (only when detailed=1)")
    school_id: Optional[StrictInt] = Field(default=None, description="School ID for FilTeam/Deal favorites (only when detailed=1)")
    sport_name: Optional[StrictStr] = Field(default=None, description="Sport name for FilTeam favorites (only when detailed=1)")
    __properties: ClassVar[List[str]] = ["id", "favoritable_id", "favorites_category_id", "category_name", "name", "school_id", "sport_name"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.favorites_category_id is None and "favorites_category_id" in self.model_fields_set:
