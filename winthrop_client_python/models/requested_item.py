...
class RequestedItem(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["id", "foia_request_id", "requestable_id", "requestable_type", "received", "created_at", "updated_at", "coach_id", "status"]

    @field_validator('requestable_type')
    def requestable_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DealStatus', 'Game', 'Compensation', 'IncomeReport', 'NcaaFinancialReportStatus', 'AuditedFinancialReportStatus']):
            raise ValueError("must be one of enum values ('DealStatus', 'Game', 'Compensation', 'IncomeReport', 'NcaaFinancialReportStatus', 'AuditedFinancialReportStatus')")
        return value
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(['pending', 'not_available', 'received']):
            raise ValueError("must be one of enum values ('pending', 'not_available', 'received')")
        return value
