...
class RawContract(BaseModel):
    ...
    text: Optional[StrictStr] = Field(
        default=None,
        description="Current Mistral OCR text. Null for legacy rows that have not been OCR'd by Mistral (see `gs_text`).",
    )
    gs_text: Optional[StrictStr] = Field(
        default=None,
        description="Legacy Ghostscript OCR text. Historical only; `text` holds the current Mistral OCR and takes precedence over this field. Null for rows created after the Mistral migration.",
    )
    ...
    unstract_responses_details: Optional[Dict[str, Any]] = None
    ...
    school_name: Optional[StrictStr] = Field(
        default=None,
        description="Short or full name of the school the raw contract is tied to (matches school_id). Null when no school is linked.",
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if gs_text (nullable) is None
        # and model_fields_set contains the field
        if self.gs_text is None and "gs_text" in self.model_fields_set:
            _dict["gs_text"] = None
