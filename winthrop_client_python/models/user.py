...
class User(BaseModel):
    ...
    can_see_eada_financials: Optional[StrictBool] = Field(
        default=None,
        description="Whether the user can access the normalized EADA institution/sport financials API (WINAD-10370). A class-level check; per-school visibility (e.g. division-scoped subscriptions, private-school access) is still enforced per-request.",
    )
    ...
    can_show_desk: Optional[StrictBool] = Field(
        default=None,
        description="Whether The Desk exists for this user (WINAD-10415 / D-29): super admin, or at least one LIVE report published to their account or to every school. One flag gates the whole feature — nav entry, /desk/* routes, gallery, reader, downloads, archive and every ask affordance. Computed per request, so a publish or a hide shows on the next fetch. ",
    )
    can_launch_intercollegiate: Optional[StrictBool] = Field(
        default=None,
        description="Whether the user can launch Intercollegiate direct access",
    )
    intercollegiate_url: Optional[StrictStr] = Field(
        default=None,
        description="Final Intercollegiate launch or marketing URL for the user",
    )
    ...
    school_name: Optional[StrictStr] = Field(
        default=None,
        description="Viewer's own school name (school accounts only) — the scheduling Message dialog sender identity",
    )
    school_logo_url: Optional[StrictStr] = Field(
        default=None,
        description="Viewer's own school logo URL (small variant); null when no school or no logo — dialog falls back to initials",
    )
    ...
    scheduling_phone: Optional[StrictStr] = Field(
        default=None,
        description="User-controlled scheduling phone number (textable cell preferred), stored separately from scraped coach phone and never overwritten by directory imports",
    )
    scheduling_phone_default: Optional[StrictStr] = Field(
        default=None,
        description="Existing phone number on file used to prefill the Edit Profile scheduling-phone field when no scheduling phone has been saved",
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # and model_fields_set contains the field
        if (
            self.intercollegiate_url is None
            and "intercollegiate_url" in self.model_fields_set
        ):
        ...
        # set to None if scheduling_phone_default (nullable) is None
        # and model_fields_set contains the field
        if (
            self.scheduling_phone_default is None
            and "scheduling_phone_default" in self.model_fields_set
        ):
            _dict["scheduling_phone_default"] = None
        ...
        _obj = cls.model_validate(
            {
                ...
                "can_show_desk": obj.get("can_show_desk"),
                ...
                "scheduling_phone_default": obj.get("scheduling_phone_default"),
