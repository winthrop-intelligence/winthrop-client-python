...
class GamePostSearchResult(BaseModel):
    ...
    posts: Optional[List[GamePostSearchResultPostsInner]] = Field(
        default=None,
        description="WINAD-9909: the posting school's own active Games Wanted posts for this sport (one day per post). These are the card's date chips. The feed is grouped one row per school, so this aggregates every post for the school.",
    )
