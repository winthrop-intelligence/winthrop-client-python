...
class GamePostSearchResult(BaseModel):
    ...
    publish_group_id: Optional[UUID] = Field(
        default=None,
        description='Identifies the publish (one "Post game wanted" action) this card represents. Shared by every post created in the same publish, so the grouped feed renders one card per publish. Null for legacy posts.',
    )
    school_id: Optional[StrictInt] = None
    school_name: Optional[StrictStr] = None
    schedule_profile_eligible: Optional[StrictBool] = Field(
        default=None,
        description='WINAD-10097 - whether the posting school has a supported D1/D2 schedule profile. When false the card omits its "View school profile" / school-name links to /schedules/:sport/:school_id.',
    )
    ...
    guarantee: Optional[GamePostSearchResultGuarantee] = None
        ...
        _obj = cls.model_validate(
            {
                ...
                "overlap": (
                    GamePostSearchResultOverlap.from_dict(obj["overlap"])
                    if obj.get("overlap") is not None
                    else None
                ),
                "contact": (
                    GamePostSearchResultContact.from_dict(obj["contact"])
                    if obj.get("contact") is not None
                    else None
                ),
                "guarantee": (
                    GamePostSearchResultGuarantee.from_dict(obj["guarantee"])
